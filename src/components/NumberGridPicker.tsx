...
export const NumberGridPicker: React.FC<NumberGridPickerProps> = ({
    isOpen,
    selectedNumbers,
    onChange,
    onClose,
}) => {
    // Defensive fallback — never let an undefined/missing prop crash the
    // whole PoolsModal (all four picker instances mount unconditionally).
            ...
            >
                ...
                <div className="px-5 pt-4 pb-3 border-b border-neutral-200">
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h2 className="text-lg font-black text-neutral-900">Pick numbers</h2>
                            <p className="text-xs text-neutral-400 mt-1">
                                Click to add • Right-click to remove one
                            </p>
                        </div>
                        ...
                        {ALL_NUMBERS.map((number) => {
                            ...
                            return (
                                <button
                                    key={number}
                                    type="button"
                                    onClick={() => addOne(number)}
                                    onContextMenu={(event) => {
                                        event.preventDefault();
                                        removeOne(number);
                                    }}
                                    className={`relative aspect-square rounded-full flex items-center justify-center text-xs sm:text-sm font-extrabold border transition-all select-none ${active
                                    ...
                                    title={active ? 'Click to add another • Right-click to remove' : 'Click to add'}
                                >
                                    {number}
                                    {count > 1 && (
